...
import="cn.sh.guanghua.util.tools.ParamTools,
        ...
        java.util.List,
        cn.sh.guanghua.mediastack.dataunit.*"%><%@ include file="cookies.jsp" %><%
     ...
     * 二是根据读到的结果显示不同的内容
     */
    String userInfo = "testUserOnly";
    String sessionId = "testUserOnly";
    String bingindId = "testUserOnly";
...
try{
    ...
    if(clipId == -1){
        List clips = Db.MediaClip().getMediaClipsByMediaid(mediaId);
        if(clips != null ){
            if(clips.size()>0){
                MediaClip clip =(MediaClip) clips.get(0);
                if(clip!= null){
                    clipId = clip.getMediaclipId();
                }else{
                  out.println("clip info searched is null!");
                  return;
                }
            }else{
              out.println("0 clip with mediaId:"+mediaId);
              return;
            }
        }else{
            out.println("No any clip with mediaId:"+mediaId);
            return;
        }
...
//免费影片（费用为0）的影片直接播放
    ...
    String playerUrl ="account/Player.jsp?"+paramString;
    response.sendRedirect(playerUrl);
	return;
    //out.println(playerUrl);
}catch(Exception e){
    e.printStackTrace();
}
