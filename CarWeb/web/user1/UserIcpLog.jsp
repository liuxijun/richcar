...
%><%@ page import="java.util.Date,
 ...
 contentType="text/html;charset=gb2312" %><%
    ...
    //定义数据访问变量
     String mediaName = ParamTools.getParameter(request,"medianame3","");
     Date startTime1 = ParamTools.getDateParameter(request,"start_time3",StringTools.date2string(System.currentTimeMillis()-24L*3600L*1000L));
     long startTime = startTime1.getTime();
     Date endTime1   = ParamTools.getDateParameter(request,"end_time3",StringTools.date2string(System.currentTimeMillis()));
     long endTime = endTime1.getTime();
     long icpId     = ParamTools.getLongParameter(request,"icp_id3",-1);
     long impId     = ParamTools.getLongParameter(request,"imp_id3",-1);
     long channelId     = ParamTools.getLongParameter(request,"channel_id3",-1);
     String orderBy = ParamTools.getParameter(request,"order_by3","mlh.medialogIcpid");
     String orderDirectory = ParamTools.getParameter(request,"order_dir3","desc");
     ...
     searchHelper = UserLogLogic.getUserIcpLog(channelId,userid,startTime,endTime,icpId,
             orderBy,orderDirectory,pageHelper.getStartRow(),pageHelper.getRowCountPerPage());
...
//显示参数
     ...
     for(int i=0;i<resultList.size();i++){
         ...
         String dataStr = PageHelper.addElement("icp-name",objs[0]);
