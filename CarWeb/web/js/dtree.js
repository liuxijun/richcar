...
function dTree(objName) {
	...
	this.aNodes = [];
	this.aIndent = [];
	this.root = new Node(-1,null,null,null,null,null,null,null,null);
...
//整理树状结构，找到没有父节点的数据，并设置到根
dTree.prototype.checkRoot = function(){
    var i=0;
    var j=0;
    for(i=0;i<this.aNodes.length;i++){
        var aNode = this.aNodes[i];
        var nodeHasParent = false;
        if(aNode!=null && aNode.pid!=-1){
            for(j=0;j<this.aNodes.length;j++){
                if(i==j) continue;
                var pNode = this.aNodes[j];
                if(pNode.id == aNode.pid){
                    nodeHasParent = true;
                    break;
                }
            }
...
dTree.prototype.node = function(node, nodeId) {
    ...
    //alert(node.name+" useCheckbox");
    if(this.config.useCheckbox){
        str += '<input type="checkbox" name="'+node.id+'" value="' + node.name + '"/>';
    }
	if (node.url) {
