<%@ page import="com.fortune.rms.business.content.model.ContentFile" %><%@ page import="java.util.ArrayList" %><%@ page import="com.fortune.util.JsonUtils" %><%@ page import="com.fortune.rms.business.content.model.Content" %><%--
  ...
  Time: 10:18:11
  发布视频的选择或上传文件
--%><%@ taglib prefix="s" uri="/WEB-INF/tlds/struts-tags.tld" %><%@ page contentType="text/html;charset=UTF-8" language="java" %><%
    ...
    String innerIp=request.getServerName();
    int innerPort=request.getServerPort();
    if("weichai".equals(AppConfigurator.getInstance().getConfig("system.lsbMethod",null))){
        innerIp = getIpFromPolicy(request.getRemoteAddr());
        innerPort=8080;
        //如果不是内网的IP段，就是用外网的IP
        if(!"10.0.66.11".equals(innerIp)){
            innerIp = "119.191.61.22";
        }
    }
    boolean showSelectModule = AppConfigurator.getInstance().getBoolConfig("system.showSelectModule",false);
%><%@include file="../user/urlUtils.jsp"%><!DOCTYPE html>
...
<head>
    ...
    <script type="text/javascript">
        ...
        var showSelectModule = <%=showSelectModule%>;
        pubFileOptions.innerIp = '<%=innerIp%>';
        pubFileOptions.innerPort = <%=innerPort%>;
...
%>                <div class="form-group">
                <label class="col-sm-2 control-label no-padding-right filed-need">资源所属的类别</label>
...
%><input type="hidden" id="movie-type" name="obj.moduleId" value="503754003"/>
                <%
    }
%>
                                    ...
                                    <div>
                                        <table class="table table-striped table-bordered table-hover" id="sample-table-3">
                                            ...
                                            <tr><td colspan='4' align="left" ><%
                                                if(AppConfigurator.getInstance().getBoolConfig("system.showManualAddButton",false)){
%><span class="btn btn-blue" style="float:left" id="addFileManual" onclick="pubFileOptions.showAddFileDialog()">手工添加</span><%
                                                }
                                            %><div class="col-md-6 col-md-offset-4"><ul class="pagination pull-right" id="page-nav"></ul></div></td></tr>
                                         </table>
                                    </div>
    ...
    jQuery(function ($) {
        ...
        $.ajax({
            ...
            success: function(msg){
                ...
                if (response.objs) {
                    var movieTypes = $("#movie-type");
                    for (var n = 0; n < response.objs.length; n++) {
                        var cn = response.objs[n];
                        if(n==0&&!showSelectModule){
                            movieTypes.val(cn.id);
                        }
                        var selected = false;
                        if(cn.code=='videoNews'){
                            selected = true;
                            if(!showSelectModule){
                                movieTypes.val(cn.id);
                                break;
