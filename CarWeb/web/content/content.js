var properties = new Array;// 所有属性的集合
var propertySelects = new Array;//属性的可选项
var formObjects = new Array;//表单控件
...
function fortuneAppend(dst, properties) {
    if (dst == null) {
        dst = {};
    }
    if (properties) {
...
FortuneBaseSingleFormItem = function(p, form1, no, pno, properties) {
    this.id = p.id + '_' + pno;
    ...
    if (properties) {
        for (var pr in properties) {
            this[pr] = properties[pr];
        }
    }
};
FortuneBaseMultiFormItem = function(p, form1, no, pno, properties,withCopy,withDel,withViewWmv) {
    var valueItem = new FortuneBaseSingleFormItem(p, form1, no, pno, properties);
...
function getStreamMediaProperties(p,labelWidth,pno,inputWidth){
    return {
        xtype:'panel',
        width:'100%',
        layout:'table',
...
function addFormItem(p, form1, no) {
    ...
    switch (p.dataType) {
        ...
        //todo 2  文本多行
        case 2:
            properties = {xtype:'textarea',width:320};
            break;
        //todo 3  文本数字
        case 3:
            break;
        //todo 4 文本日期
        case 4:
            properties = {xtype:'datefield',labelWidth:labelWidth,width:320};
            break;
        ...
        //todo 8 file wmv
        case 8:
            properties = getStreamMediaProperties();
            break;
    ...
    if (p.isMultiLine == 0) {
        addElement(formObjects, new FormObject(p.id + '_' + pno, p,
                new FortuneBaseSingleFormItem(p, form1, no, pno, properties)
                ));
    } else {
        addElement(formObjects, new FormObject(p.id + '_' + pno, p, new FortuneBaseMultiFormItem(p, form1, no, pno, properties)));
    }
