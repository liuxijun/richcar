...
<%
    ...
    String actionHeader = "admin";
    {
        ...
        needPermissions(actionHeader,"spOffline","contentSpOffline,SpOfflineContentManage");
        needPermissions(actionHeader,"OfflineView","SpOfflineContentManage");
        needPermissions(actionHeader,"spDelete","contentSpDelete,SpOfflineContentManage");
        needPermissions(actionHeader,"spRecycle","contentSpRecycle,SpOfflineContentManage");
        needPermissions(actionHeader,"spSearch","systemContentSpSearch,SpOfflineContentManage");
    }
...
Ext.onReady(function(){
    ...
    var dataListStore = new Ext.data.JsonStore({
            method:'POST',
            remoteSort: true,
            url: "/content/content!spSearch.action?c_status=2&cc_status=1&cc_cspId="+spId,
                                ...
                                Ext.MessageBox.confirm("请您确认操作", " 申请上线 ",function(btn){
                                    if(btn=="yes"){
                                        ...
                                        remoteRequestStore.reload({
                                            params:{keyIds:keyIds,spId:spId, status:3},
                                            callback :
                                                    function(records,options,success){
    ...
    var listGrid = new Ext.grid.GridPanel({
        ...
        columns: [
            checkSelect,
                {header: "ID", dataIndex: 'c_id',hidden:true, width: 50, sortable: true, align:'center'},
                {header: "名称", dataIndex: 'c_name', width: 130, sortable: true, align:'center',
                    renderer:
                        function (val,p,row){
                            if(canDoThisAction('OfflineView'))          {
                            return '<a href=\'javascript:viewObj("view","'+row.data.c_id+'",'+row.data.c_moduleId+')\'>'+ val +'</a>';
                        }            }
                },

                {header: "主演", dataIndex: 'c_actors', width: 100, sortable: true, align:'center'},
