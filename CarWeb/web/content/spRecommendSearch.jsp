...
//        needPermissions(actionHeader,"list","contentRecommendList");
...
//        needPermissions(actionHeader,"searchAll","channelSearchAll");
        needPermissions(actionHeader,"addRecommendContent","contentAddRecommendContent");
        needPermissions(actionHeader,"RecommendSearchView","SpRecommendContentManage");
        needPermissions(actionHeader,"spRecommendSearch","contentSpRecommendSearch");
...
%>
    ...
    var recSearchCon="?obj.recommendId="+recommendId;
    if (recommendType==2){
        recSearchCon+="&obj.channelId="+channelId;
    }
    ...
    var contentRecommendStore = new Ext.data.JsonStore({
         method:'POST',
         url:"/content/contentRecommend!list.action" + recSearchCon,
         baseParams:{limit:1000000},
         totalProperty:'totalCount',
         root:'objs',
         fields:[
                ...
                items:[
                    {
                        ...
                        listeners:{
                            "click":function()
                            {
                                var keyIds = getCheckboxValue("contentId");
                                if (keyIds==''){
                                    Ext.Msg.alert("提示","未选择记录！");
                                    return;
                                }
                                Ext.MessageBox.confirm("请您确认操作", " 添加资源推荐 ",function(btn){
                                    if(btn=="yes"){

                                        var remoteRequestStore = new Ext.data.JsonStore({
                                                method:'POST',
                                                url: actionUrl + "!addRecommendContent.action"
                                        });
                                        remoteRequestStore.reload({
                                            params:{keyIds:keyIds,recommendType:recommendType,recommendId:recommendId,channelId:channelId},
                                            callback :
                                                    function(records,options,success){
                ...
                items: [
 {
                        ...
                        items: [
                            {
                                ...
                                listeners:{
                                    select: function(combo, record, index) {
                                    //  alert(index +":"+ record.get("id") +":"+ record.get("name"));
    ...
    var listGrid = new Ext.grid.GridPanel({
        ...
        sm:checkSelect,
        columns: [
                {header: "选择", dataIndex: 'c_id', width: 50, sortable: true, align:'left',
                     renderer:function(val,p,row){
                        var flag=true;
                        for (var i=0; i<contentRecommendStore.getCount(); i++){
                            var storeRecord = contentRecommendStore.getAt(i);
                            if (storeRecord.data.contentId==val){
                                flag=false;
                                break;
                            }
                        }
                         if (flag){
                             return '<input type="checkbox" name="contentId" value="'+val+'"/>';
                         }
                    }
                },
                ...
                {header: "名称", dataIndex: 'c_name', width: 130, sortable: true, align:'left',
                    renderer:
                        function (val,p,row){
                            if(canDoThisAction('RecommendSearchView')){
                ...
                {header: "CP", dataIndex: 'c_cspId', width: 120, sortable: true, align:'center',
                    renderer:function(val,p,row){
                ...
                {header: "设备", dataIndex: 'c_deviceId', width: 110, sortable: true, align:'center',
                    renderer:function(val,p,row){
