...
*/
                ...
                final String[] propertyCodes = new String[]{"Media_Url_Source"};
...
*/
                        ...
                        allClips.addAll(contentPropertyLogicInterface.getContentProperties(-1L,propertyIds));
        ...
        return new BatchRunnable() {
            ...
            public void run() {
                ...
                for(ContentProperty clip:clips){
                    ...
                    if(mediaFile.exists()){
                        if(mediaFile.length()>1024*128){//至少大于1MB
///*
                    SimpleFileInfo simpleFileInfo = new SimpleFileInfo(mediaFile);
                            if(FileUtils.setFileMediaInfo(mediaFile.getAbsolutePath(),simpleFileInfo)){
                                int clipDuration = simpleFileInfo.getLength();
                                if(clipDuration>0){
                                    if(clipDuration!=clip.getLength()){
                                        log+="\r\n--媒体"+content.getName()+"的片段"+clip.getName()+","+clip.getIntValue()+"时长进行修复：";
                                        log+="\r\nupdate CONTENT_PROPERTY set LENGTH="+simpleFileInfo.getLength()+" where id="+clip.getId()+";";
                                        outputLines++;
                                    }
                                    resultCode = 200;
                                    mediaLength+=clipDuration;
                                }else{
                                    resultCode = 500;
                                }
                            }else{
...
//                            resultCode = 200;
                ...
                ContentProperty mediaLengthContentProperty = new ContentProperty();
                mediaLengthContentProperty.setContentId(content.getId());
                mediaLengthContentProperty.setPropertyId(mediaLengthProperty.getId());
                List<ContentProperty> oldMediaLengths=contentPropertyLogicInterface.search(mediaLengthContentProperty);
                int l=0;
                if(oldMediaLengths!=null){
                    int i=0;
                    l=oldMediaLengths.size();
                    for(;i<l;i++){
                        ContentProperty cp=oldMediaLengths.get(i);
                        if(i>0){
                            log+="\r\n--删除这个重复的mediaLength数据\r\ndelete from media_property where id="+cp.getId();
                        }else{
                            log+="\r\nupdate content_property set int_value="+mediaLength+",string_value='"+mediaLength+"' where id="+cp.getId();
                        }
                        log+=";";
                        outputLines++;
                    }
