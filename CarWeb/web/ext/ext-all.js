...
Ext.apply(Ext.Template.prototype, {disableFormats:false,re:/\{([\w-]+)(?:\:([\w\.]*)(?:\((.*?)?\))?)?\}/g,applyTemplate:function(b) {
...
(function() {
    ...
    i.Observable.prototype = {filterOptRe:/^(?:scope|delay|buffer|single)$/,fireEvent:function() {
    ...
    i.Event.prototype = {addListener:function(p, o, n) {
        var q = this,m;
        o = o || q.obj;
            ...
            if (G.hasOwnProperty(z)) {
                ...
                for (B = 0,E = C.length; B < E; B++) {
                    ...
                    if (w && y == b && z == "mousedown") {
                        Ext.EventManager.stoppedMouseDownEvent.removeListener(w)
                    }
                }
            }
        }
...
Ext.Element.addMethods({swallowEvent:function(a, b) {
    ...
    a = Ext.isObject(a) ? a : {tag:"div",cls:a};
...
Ext.Element.addMethods(function() {
    ...
    return{originalDisplay:"",visibilityMode:1,setVisibilityMode:function(l) {
        g(this.dom, c, l);
        return this
        ...
        return !l[m] ? false : (Ext.isObject(l[m]) ? l[m] : {duration:l[m + 1],callback:l[m + 2],easing:l[m + 3]})
    },isVisible:function() {
...
Ext.extend(Ext.CompositeElement, Ext.CompositeElementLite, {getElement:function(a) {
    return a
},transformElement:function(a) {
    return Ext.get(a)
}});
...
Ext.extend(Ext.util.MixedCollection, Ext.util.Observable, {allowFunctions:false,add:function(b, c) {
    ...
    for (e = 0,g = h.length; e < g; e++) {
        j[j.length] = {key:b[e],value:h[e],index:e}
    }
    j.sort(function(i, c) {
        var k = l(i[m], c[m]) * d;
        if (k === 0) {
...
Ext.XTemplate = function() {
    ...
    while ((q = i.match(p))) {
        ...
        if (x) {
            switch (x) {case".":x = new Function(n, u, c + e + n + "; }");break;case"..":x = new Function(n, u, c + e + u + "; }");break;default:x = new Function(n, u, c + e + x + "; }")
            }
        }
        j.push({id:o,target:x,exec:r,test:h,body:q[1] || ""});
...
Ext.extend(Ext.Component, Ext.util.Observable, {disabled:false,hidden:false,autoEl:"div",disabledClass:"x-item-disabled",allowDomMove:true,autoShow:false,hideMode:"display",hideParent:false,rendered:false,tplWriteMode:"overwrite",ctype:"Ext.Component",actionMode:"el",getActionEl:function() {
...
Ext.Container = Ext.extend(Ext.BoxComponent, {bufferResize:50,autoDestroy:true,forceLayout:false,defaultType:"panel",resizeEvent:"resize",bubbleEvents:["add","remove"],initComponent:function() {
    ...
    var b = a.render ? a : Ext.create(Ext.apply({ownerCt:this}, a), d || this.defaultType);
    delete b.ownerCt;
    return b
},canLayout:function() {
...
Ext.layout.AutoLayout = Ext.extend(Ext.layout.ContainerLayout, {runLayout:function() {
    var a = this.container;
    a.doLayout();
    delete a.layoutPending
}});
...
Ext.layout.CardLayout = Ext.extend(Ext.layout.FitLayout, {deferredRender:false,layoutOnCardChange:false,renderHidden:true,type:"card",constructor:function(a) {
...
Ext.layout.AnchorLayout = Ext.extend(Ext.layout.ContainerLayout, {monitorResize:true,type:"anchor",getLayoutTargetSize:function() {
    var a = this.container.getLayoutTarget();
    if (!a) {
        return{}
    }
    ...
    for (j = 0; j < m; j++) {
        ...
        if (p.anchor) {
            ...
            g = r.right ? this.adjustWidthAnchor(r.right(s) - e.getMargins("lr"), p) : undefined;
            b = r.bottom ? this.adjustHeightAnchor(r.bottom(k) - e.getMargins("tb"), p) : undefined;
            if (g || b) {
...
Ext.layout.ColumnLayout = Ext.extend(Ext.layout.ContainerLayout, {monitorResize:true,type:"column",extraCls:"x-column",scrollOffset:0,targetCls:"x-column-layout-ct",isValidParent:function(b, a) {
    ...
    for (a = 0; a < g; a++) {
        l = e[a];
        if (l.columnWidth) {
            l.setSize(Math.floor(l.columnWidth * n) - l.getPositionEl().getMargins("lr"))
        }
    }
    if (Ext.isIE) {
        if (a = j.getStyle("overflow") && a != "hidden" && !this.adjustmentPass) {
...
Ext.layout.FormLayout = Ext.extend(Ext.layout.AnchorLayout, {labelSeparator:":",trackLabels:false,type:"form",onRemove:function(d) {
    ...
    return{id:b.id,label:b.fieldLabel,labelStyle:this.getLabelStyle(b.labelStyle),elementStyle:this.elementStyle || "",labelSeparator:a ? "" : (Ext.isDefined(b.labelSeparator) ? b.labelSeparator : this.labelSeparator),itemCls:(b.itemCls || this.container.itemCls || "") + (b.hideLabel ? " x-hide-label" : ""),clearCls:b.clearCls || "x-form-clear-left"}
...
Ext.layout.AbsoluteLayout = Ext.extend(Ext.layout.AnchorLayout, {extraCls:"x-abs-layout-item",type:"anchor",onLayout:function(a, b) {
    b.position();
...
Ext.layout.VBoxLayout = Ext.extend(Ext.layout.BoxLayout, {align:"left",type:"vbox",onLayout:function(j, K) {
    Ext.layout.VBoxLayout.superclass.onLayout.call(this, j, K);
    var k = this.getRenderedItems(j),m = k.length,I,D,o,s,z,p,v,r,e,A = this.getLayoutTargetSize(),q = A.width,F = A.height - this.scrollOffset,B = this.padding.left,u = this.padding.top,n = this.pack == "start",x = 0,H = 0,C = 0,g = 0,y = 0,G = [],E = [];
    for (D = 0; D < m; D++) {
        I = k[D];
        o = I.margins;
        z = o.top + o.bottom;
    ...
    switch (this.align) {case"stretch":this.innerCt.setSize(q, F);break;case"stretchmax":case"left":this.innerCt.setSize(a, F);break;case"center":this.innerCt.setSize(q = Math.max(q, a), F);break
    }
    ...
    for (D = 0; D < m; D++) {
        ...
        if (this.align == "stretch") {
            I.setWidth(((q - (this.padding.left + this.padding.right)) - (o.left + o.right)).constrain(I.minWidth || 0, I.maxWidth || 1000000))
        } else {
            if (this.align == "stretchmax") {
                I.setWidth((H - (o.left + o.right)).constrain(I.minWidth || 0, I.maxWidth || 1000000))
            } else {
                if (n && I.flex) {
                    I.setWidth()
                }
...
Ext.layout.HBoxLayout = Ext.extend(Ext.layout.BoxLayout, {align:"top",type:"hbox",onLayout:function(e, J) {
    Ext.layout.HBoxLayout.superclass.onLayout.call(this, e, J);
    var g = this.getRenderedItems(e),j = g.length,H,D,n,d,q,s,A,v = this.getLayoutTargetSize(),p = v.width - this.scrollOffset,F = v.height,y = this.padding.left,r = this.padding.top,m = this.pack == "start",C = ["stretch","stretchmax"].indexOf(this.align) == -1,G = 0,x = 0,z = 0,k = 0;
    for (D = 0; D < j; D++) {
        H = g[D];
        z += H.flex || 0;
        if (!H.flex) {
    ...
    switch (this.align) {case"stretch":this.innerCt.setSize(p, F);break;case"stretchmax":case"top":this.innerCt.setSize(p, B);break;case"middle":this.innerCt.setSize(p, F = Math.max(F, B));break
    }
    var I = A,o = [],E = [],u = 0,b = Math.max(0, F - this.padding.top - this.padding.bottom);
    for (D = 0; D < j; D++) {
        H = g[D];
        if (m && H.flex) {
    ...
    for (D = 0; D < j; D++) {
        ...
        if (this.align == "stretch") {
            H.setHeight(((F - (this.padding.top + this.padding.bottom)) - (n.top + n.bottom)).constrain(H.minHeight || 0, H.maxHeight || 1000000))
        } else {
            if (this.align == "stretchmax") {
                H.setHeight((x - (n.top + n.bottom)).constrain(H.minHeight || 0, H.maxHeight || 1000000))
            } else {
...
Ext.layout.ToolbarLayout = Ext.extend(Ext.layout.ContainerLayout, {monitorResize:true,triggerWidth:18,lastOverflow:false,noItemsMenuText:'<div class="x-toolbar-no-items">(None)</div>',onLayout:function(d, h) {
    if (!this.leftTr) {
        var g = d.buttonAlign == "center" ? "center" : "left";
        h.addClass("x-toolbar-layout-ct");
        h.insertHtml("beforeEnd", '<table cellspacing="0" class="x-toolbar-ct"><tbody><tr><td class="x-toolbar-left" align="' + g + '"><table cellspacing="0"><tbody><tr class="x-toolbar-left-row"></tr></tbody></table></td><td class="x-toolbar-right" align="right"><table cellspacing="0" class="x-toolbar-right-ct"><tbody><tr><td><table cellspacing="0"><tbody><tr class="x-toolbar-right-row"></tr></tbody></table></td><td><table cellspacing="0"><tbody><tr class="x-toolbar-extras-row"></tr></tbody></table></td></tr></tbody></table></td></tr></tbody></table>');
    ...
    var m = n.dom.clientWidth,b = this.lastWidth || 0,d = n.dom.firstChild.offsetWidth,l = m - this.triggerWidth,k = -1;
    this.lastWidth = m;
    ...
    Ext.apply(a, {text:e.overflowText || e.text,iconCls:e.iconCls,icon:e.icon,itemId:e.itemId,disabled:e.disabled,handler:e.handler,scope:e.scope,menu:e.menu,hideOnClick:b});
    if (d || e.enableToggle) {
        Ext.apply(a, {group:d,checked:e.pressed,listeners:{checkchange:function(g, c) {
            e.toggle(c)
        }}})
    }
    delete a.ownerCt;
...
Ext.layout.MenuLayout = Ext.extend(Ext.layout.ContainerLayout, {monitorResize:true,setContainer:function(a) {
...
Ext.Panel = Ext.extend(Ext.Container, {baseCls:"x-panel",collapsedCls:"x-panel-collapsed",maskDisabled:true,animCollapse:Ext.enableFx,headerAsText:true,buttonAlign:"right",collapsed:false,collapseFirst:true,minButtonWidth:75,elements:"body",preventBodyReset:false,padding:undefined,resizeEvent:"bodyresize",toolTarget:"header",collapseEl:"bwrap",slideAnchor:"t",disabledClass:"",deferHeight:true,expandDefaults:{duration:0.25},collapseDefaults:{duration:0.25},initComponent:function() {
    ...
    if (this.rendered && this.header) {
        if (this.frame) {
            ...
            if (c) {
                ...
                Ext.DomHelper.insertBefore(d.dom.firstChild, {tag:"img",src:Ext.BLANK_IMAGE_URL,cls:"x-panel-inline-icon " + this.iconCls})
            }
        }
    }
    ...
    if (Ext.isDefined(a) || Ext.isDefined(b)) {
        if (!this.collapsed) {
            ...
            if (Ext.isNumber(b)) {
                b = Math.max(0, this.adjustBodyHeight(b - this.getFrameHeight()));
...
Ext.extend(Ext.Editor, Ext.Component, {value:"",alignment:"c-c?",offsets:[0,0],shadow:"frame",constrain:false,swallowKeys:true,completeOnEnter:true,cancelOnEsc:true,updateEl:false,initComponent:function() {
    ...
    if (this.autoSize) {
        ...
        switch (this.autoSize) {case"width":this.setSize(b.width, a.height);break;case"height":this.setSize(a.width, b.height);break;case"none":this.setSize(a.width, a.height);break;default:this.setSize(b.width, b.height)
        }
    }
...
Ext.DatePicker = Ext.extend(Ext.BoxComponent, {todayText:"Today",okText:"&#160;OK&#160;",cancelText:"Cancel",todayTip:"{0} (Spacebar)",minText:"This date is before the minimum date",maxText:"This date is after the maximum date",format:"m/d/y",disabledDaysText:"Disabled",disabledDatesText:"Disabled",monthNames:Date.monthNames,dayNames:Date.dayNames,nextText:"Next Month (Control+Right)",prevText:"Previous Month (Control+Left)",monthYearText:"Choose a month (Control+Up/Down to move years)",startDay:0,showToday:true,focusOnSelect:true,initComponent:function() {
    ...
    if (this.rendered) {
        ...
        var k = H.getDaysInMonth(),q = H.getFirstDateOfMonth(),g = q.getDay() - this.startDay;
        if (g < 0) {
            g += 7
        }
        k += g;
        ...
        if (this.showToday) {
            var m = new Date().clearTime(),c = (m < u || m > y || (G && A && G.test(m.dateFormat(A))) || (J && J.indexOf(m.getDay()) != -1));
            if (!this.disabled) {
...
Ext.Slider = Ext.extend(Ext.BoxComponent, {vertical:false,minValue:0,maxValue:100,decimalPrecision:0,keyIncrement:1,increment:0,clickRange:[5,15],clickToChange:true,animate:true,dragging:false,initComponent:function() {
...
Ext.Slider.Vertical = {onResize:function(a, b) {
    ...
    var c = this.innerEl.translatePoints(this.tracker.getXY()),a = this.innerEl.getHeight() - c.top;
...
Ext.data.Api = (function() {
    ...
    return{actions:{create:"create",read:"read",update:"update",destroy:"destroy"},restActions:{create:"POST",read:"GET",update:"PUT",destroy:"DELETE"},isAction:function(b) {
        ...
        b.onWrite = b.onWrite.createInterceptor(function(i, j, g, e) {
            ...
            switch (g.status) {case 200:return true;break;case 201:if (Ext.isEmpty(h.raw.responseText)) {
                h.success = true
            } else {
                return true
            }break;case 204:h.success = true;h.data = null;break;default:return true;break
            }
            if (h.success === true) {
...
Ext.data.Store = Ext.extend(Ext.util.Observable, {writer:undefined,remoteSort:false,autoDestroy:false,pruneModifiedRecords:false,lastOptions:null,autoSave:true,batch:true,restful:false,paramNames:undefined,defaultParamNames:{start:"start",limit:"limit",sort:"sort",dir:"dir"},batchKey:"_ext_batch_",constructor:function(a) {
    ...
    switch (c) {case"json":a = Ext.data.JsonWriter;break;case"xml":a = Ext.data.XmlWriter;break;default:a = Ext.data.JsonWriter
    }
    return new a(b)
},destroy:function() {
    if (!this.isDestroyed) {
...
Ext.data.DataReader.prototype = {getTotal:Ext.emptyFn,getRoot:Ext.emptyFn,getMessage:Ext.emptyFn,getSuccess:Ext.emptyFn,getId:Ext.emptyFn,buildExtractors:Ext.emptyFn,extractData:Ext.emptyFn,extractValues:Ext.emptyFn,realize:function(a, c) {
    if (Ext.isArray(a)) {
        for (var b = a.length - 1; b >= 0; b--) {
            if (Ext.isArray(c)) {
...
Ext.extend(Ext.data.XmlReader, Ext.data.DataReader, {read:function(a) {
    ...
    var a = Ext.DomQuery;
    return function(b) {
        switch (b) {case this.meta.totalProperty:return function(c, d) {
            return a.selectNumber(b, c, d)
        };break;case this.meta.successProperty:return function(d, e) {
            var c = a.selectValue(b, d, true);
            var g = c !== false && c !== "false";
            return g
        };break;default:return function(c, d) {
...
Ext.data.GroupingStore = Ext.extend(Ext.data.Store, {constructor:function(a) {
    ...
    if (this.remoteGroup) {
        ...
        var a = this.lastOptions;
        if (a && a.params) {
            Ext.apply(a.params, {groupBy:this.groupField,groupDir:this.groupDir})
        }
    }
},applySort:function() {
...
Ext.extend(Ext.data.DirectProxy, Ext.data.DataProxy, {paramOrder:undefined,paramsAsHash:true,directFn:undefined,doRequest:function(b, c, a, e, k, l, n) {
    ...
    switch (b) {case Ext.data.Api.actions.create:j.push(a.jsonData);break;case Ext.data.Api.actions.read:if (h.directCfg.method.len > 0) {
        if (this.paramOrder) {
    ...
    }break;case Ext.data.Api.actions.update:j.push(a.jsonData);break;case Ext.data.Api.actions.destroy:j.push(a.jsonData);break
    }
...
Ext.Resizable = Ext.extend(Ext.util.Observable, {constructor:function(d, e) {
    ...
    if (this.enabled && this.activeHandle) {
        try {
            ...
            switch (j) {case"east":m += r;m = Math.min(Math.max(n, m), s);break;case"south":u += p;u = Math.min(Math.max(A, u), D);break;case"southeast":m += r;u += p;m = Math.min(Math.max(n, m), s);u = Math.min(Math.max(A, u), D);break;case"north":p = this.constrain(u, p, A, D);k += p;u -= p;break;case"west":r = this.constrain(m, r, n, s);l += r;m -= r;break;case"northeast":m += r;m = Math.min(Math.max(n, m), s);p = this.constrain(u, p, A, D);k += p;u -= p;break;case"northwest":r = this.constrain(m, r, n, s);p = this.constrain(u, p, A, D);k += p;u -= p;l += r;m -= r;break;case"southwest":r = this.constrain(m, r, n, s);u += p;u = Math.min(Math.max(A, u), D);l += r;m -= r;break
            }
            var q = this.snap(m, i, n);
            var C = this.snap(u, a, A);
            if (q != m || C != u) {
                switch (j) {case"northeast":k -= C - u;break;case"north":k -= C - u;break;case"southwest":l -= q - m;break;case"west":l -= q - m;break;case"northwest":l -= q - m;k -= C - u;break
                }
                m = q;
                u = C
            }
...
Ext.extend(Ext.state.Provider, Ext.util.Observable, {get:function(b, a) {
    ...
    var d = g[1];
    var a = g[2];
    switch (d) {case"n":return parseFloat(a);case"d":return new Date(Date.parse(a));case"b":return(a == "1");case"a":var c = [];if (a != "") {
        Ext.each(a.split("^"), function(h) {
            c.push(this.decodeValue(h))
        }, this)
    }return c;case"o":var c = {};if (a != "") {
        Ext.each(a.split("^"), function(i) {
            var h = i.split("=");
            c[h[0]] = this.decodeValue(h[1])
        }, this)
    }return c;default:return a
    }
...
Ext.list.ListView = Ext.extend(Ext.DataView, {itemSelector:"dl",selectedClass:"x-list-selected",overClass:"x-list-over",scrollOffset:undefined,columnResize:true,columnSort:true,maxWidth:Ext.isIE ? 99 : 100,initComponent:function() {
...
Ext.list.ColumnResizer = Ext.extend(Ext.util.Observable, {minPct:0.05,constructor:function(a) {
    ...
    this.proxy.remove();
    var h = this.hdIndex,o = this.view,g = o.columns,k = g.length,q = this.view.innerHd.getWidth(),d = this.minPct * 100,r = Math.ceil((j * o.maxWidth) / q),p = (g[h].width * 100) - r,n = Math.floor(p / (k - 1 - h)),m = p - (n * (k - 1 - h));
    for (var c = h + 1; c < k; c++) {
...
Ext.ToolTip = Ext.extend(Ext.Tip, {showDelay:500,hideDelay:200,dismissDelay:5000,trackMouse:false,anchorToTarget:true,anchorOffset:0,targetCounter:0,constrainPosition:false,initComponent:function() {
    ...
    switch (this.anchor) {case"top":return"tl-bl";case"left":return"tl-tr";case"right":return"tr-tl";default:return"bl-tl"
    }
},getOffsets:function() {
    var b,a = this.getAnchorPosition().charAt(0);
    if (this.anchorToTarget && !this.trackMouse) {
        switch (a) {case"t":b = [0,9];break;case"b":b = [0,-13];break;case"r":b = [-13,0];break;default:b = [9,0];break
        }
    } else {
        switch (a) {case"t":b = [-15 - this.anchorOffset,30];break;case"b":b = [-19 - this.anchorOffset,-13 - this.el.dom.offsetHeight];break;case"r":b = [-15 - this.el.dom.offsetWidth,-13 - this.anchorOffset];break;default:b = [25,-13 - this.anchorOffset];break
        }
    }
    ...
    var a,b,c;
    switch (this.tipAnchor.charAt(0)) {case"t":a = "b";b = "tl";c = [20 + this.anchorOffset,2];break;case"r":a = "l";b = "tr";c = [-2,11 + this.anchorOffset];break;case"b":a = "t";b = "bl";c = [20 + this.anchorOffset,-2];break;default:a = "r";b = "tl";c = [2,11 + this.anchorOffset];break
    }
...
Ext.FlashComponent = Ext.extend(Ext.BoxComponent, {flashVersion:"9.0.115",backgroundColor:"#ffffff",wmode:"opaque",flashVars:undefined,flashParams:undefined,url:undefined,swfId:undefined,swfWidth:"100%",swfHeight:"100%",expressInstall:false,initComponent:function() {
    ...
    var b = Ext.apply({allowScriptAccess:"always",bgcolor:this.backgroundColor,wmode:this.wmode}, this.flashParams),a = Ext.apply({allowedDomain:document.location.hostname,elementID:this.getId(),eventHandler:"Ext.FlashEventProxy.onEvent"}, this.flashVars);
    ...
    switch (a.type) {case"swfReady":this.initSwf();return;case"log":return
    }
    a.component = this;
...
Ext.menu.Item = Ext.extend(Ext.menu.BaseItem, {itemCls:"x-menu-item",canActivate:true,showDelay:200,hideDelay:200,ctype:"Ext.menu.Item",initComponent:function() {
...
Ext.form.Checkbox = Ext.extend(Ext.form.Field, {focusClass:undefined,fieldClass:"x-form-field",checked:false,defaultAutoCreate:{tag:"input",type:"checkbox",autocomplete:"off"},actionMode:"wrap",initComponent:function() {
...
Ext.form.HtmlEditor = Ext.extend(Ext.form.Field, {enableFormat:true,enableFontSize:true,enableColors:true,enableAlignments:true,enableLists:true,enableSourceEdit:true,enableLinks:true,enableFont:true,createLinkText:"Please enter the URL for the link:",defaultLinkValue:"http://",fontFamilies:["Arial","Courier New","Tahoma","Times New Roman","Verdana"],defaultFont:"tahoma",defaultValue:(Ext.isOpera || Ext.isIE6) ? "&#160;" : "&#8203;",actionMode:"wrap",validationEvent:false,deferHeight:true,initialized:false,activated:false,sourceEditMode:false,onFocus:Ext.emptyFn,iframePad:3,hideMode:"offsets",defaultAutoCreate:{tag:"textarea",style:"width:500px;height:300px;",autocomplete:"off"},initComponent:function() {
    ...
    try {
        var c = this.getEditorBody(),a = this.el.getStyles("font-size", "font-family", "background-image", "background-repeat"),g,b;
...
Ext.grid.GridView = Ext.extend(Ext.util.Observable, {deferEmptyText:true,scrollOffset:undefined,autoFill:false,forceFit:false,sortClasses:["sort-asc","sort-desc"],sortAscText:"Sort Ascending",sortDescText:"Sort Descending",columnsText:"Columns",selectedRowClass:"x-grid3-row-selected",borderWidth:2,tdClass:"x-grid3-cell",hdCls:"x-grid3-hd",markDirty:true,cellSelectorDepth:4,rowSelectorDepth:10,rowBodySelectorDepth:10,cellSelector:"td.x-grid3-cell",rowSelector:"div.x-grid3-row",rowBodySelector:"div.x-grid3-row-body",firstRowCls:"x-grid3-row-first",lastRowCls:"x-grid3-row-last",rowClsRe:/(?:^|\s+)x-grid3-row-(first|last|alt)(?:\s+|$)/g,constructor:function(a) {
    ...
    for (var v = 0,y = k.length; v < y; v++) {
        ...
        for (var x = 0; x < q; x++) {
            ...
            u.attr = u.cellAttr = "";
            u.value = A.renderer.call(A.scope, o.data[A.name], u, o, n, x, s);
            u.style = A.style;
            if (Ext.isEmpty(u.value)) {
                u.value = "&#160;"
            }
            if (this.markDirty && o.dirty && Ext.isDefined(o.modified[A.name])) {
...
},processEvent:Ext.emptyFn,layout:function() {
    if (!this.mainBody) {
        return
    }
    var d = this.grid;
    ...
    switch (e) {case"asc":d.sort(a.getDataIndex(b), "ASC");break;case"desc":d.sort(a.getDataIndex(b), "DESC");break;default:b = a.getIndexById(e.substr(4));if (b != -1) {
...
},fixed:true,menuDisabled:true,dataIndex:"",id:"numberer",rowspan:undefined,renderer:function(b, c, a, d) {
    if (this.rowspan) {
        c.cellAttr = 'rowspan="' + this.rowspan + '"'
    }
    return d + 1
}});
Ext.grid.CheckboxSelectionModel = Ext.extend(Ext.grid.RowSelectionModel, {header:'<div class="x-grid3-hd-checker">&#160;</div>',width:20,sortable:false,menuDisabled:true,fixed:true,dataIndex:"",id:"checker",constructor:function() {
...
Ext.grid.CellSelectionModel = Ext.extend(Ext.grid.AbstractSelectionModel, {constructor:function(a) {
    ...
    switch (d) {case j.ESC:case j.PAGE_UP:case j.PAGE_DOWN:break;default:j.stopEvent();break
    }
    if (!p) {
        o = m(0, 0, 1);
        if (o) {
    ...
    a = o[0];
    l = o[1];
    switch (d) {case j.TAB:if (j.shiftKey) {
        h = m(a, l - 1, -1)
    } else {
        h = m(a, l + 1, 1)
    }break;case j.DOWN:h = m(a + 1, l, 1);break;case j.UP:h = m(a - 1, l, -1);break;case j.RIGHT:h = m(a, l + 1, 1);break;case j.LEFT:h = m(a, l - 1, -1);break;case j.ENTER:if (i.isEditor && !i.editing) {
        i.startEditing(a, l);
        return
    }break
    }
...
Ext.grid.GroupingView = Ext.extend(Ext.grid.GridView, {groupByText:"Group By This Field",showGroupsText:"Show in Groups",hideGroupedColumn:false,showGroupName:true,startCollapsed:false,enableGrouping:true,enableGroupingMenu:true,enableNoGroups:true,emptyGroupText:"(None)",ignoreAdd:false,groupTextTpl:"{text}",groupMode:"value",gidSeed:1000,initTemplates:function() {
