...
function doRenderChannel(jsonData) {
    ...
    if(sonChannels != null && sonChannels.length > 0) {
        ...
        $.each(jsonData.children,function(){
            ...
            var ch_index= "'ch"+i+"'";
            var c = this;
            var liStr = '<li id="subChannel_'+c.id+'" name="ch'+i+'"><a id="subChannel_'+c.id+'_a" href="javascript:searchChannel('+c.id+','+ch_index+')">';
            if(c.id == channelId)  {
                liStr =  '<li class="fRed" id="subChannel_'+c.id+'" name="ch'+i+'"><a id="subChannel_'+c.id+'_a" href="javascript:searchChannel('+c.id+','+ch_index+')" style="color: #c80000">';
        ...
        searchChannel(channelId,'ch'+channelIndex);
    }
