...
(function ($, undefined) {
    ...
    var Tree = function (element, options) {
        ...
        this.$element.on("click", ".tree-folder-header i", $.proxy(function (ev) {
			...
			this.selectFolder(ev.currentTarget.parentNode);
            ev.stopPropagation();
        }, this));
        this.render()
    };
            ...
            $.each(items.data, function (index, value) {
                ...
                if (value.type === "folder") {
                    ...
                    if ("additionalParameters"in value && "item-selected"in value.additionalParameters && value.additionalParameters["item-selected"] == true) {
                        setTimeout(function () {  // select folder
                            header.trigger("click")
                        }, 0)
                    }
                    if ("additionalParameters"in value && "item-opened"in value.additionalParameters && value.additionalParameters["item-opened"] == true) {
                        setTimeout(function () {  // open folder
                            header.find("i").trigger("click")
                        }, 0)
                    }
