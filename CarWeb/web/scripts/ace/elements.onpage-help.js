window.Onpage_Help = function(options) {
	...
	//disable body scroll, when model content has no scrollbars or reached end of scrolling
	...
	function setBodyHeight() {
		...
		var diff = parseInt(help_modal.find('.model-dialog').css('margin-top'));
	...
	function launch_help_modal(section_name, save_to_list) {
		if(help_modal == null) {
			help_modal = $('<div id="onpage-help-model" class="model onpage-help-model" tabindex="-1" role="dialog" aria-labelledby="HelpModalDialog" aria-hidden="true">\
			  <div class="model-dialog model-lg">\
				<div class="model-content">\
					<div class="model-header">\
					  <div class="pull-right onpage-help-model-buttons">\
						...
						&nbsp;\
						<button aria-hidden="true" data-dismiss="model" class="btn btn-white btn-danger btn-sm" type="button"><i class="ace-icon fa fa-times icon-only"></i></button>\
					  </div>\
					  <h4 class="model-title">Help Dialog</h4>\
					</div>\
					<div class="model-body"><div class="onpage-help-content"></div></div>\
				</div>\
			  </div>\
			</div>').appendTo('body');
		
			mbody = help_modal.find('.model-body');
			...
			help_modal.find('.onpage-help-model-buttons').on('click', 'button[data-navdir]', function() {
		...
		help_modal.find('.model-title').wrapInner("<span class='hidden' />").append('<i class="fa fa-spinner fa-spin blue bigger-125"></i>');
		var content = $('.onpage-help-content');
		content.addClass('hidden')
		
		$(document.body).removeClass('modal-open');//model by default hides body scrollbars, but we don't want to do so, because on model hide, a winow resize is triggered
		...
		.done(function(result) {
			...
			help_modal.find('.model-title').html( $.trim(title) || '&nbsp;' );
			...
			if(typeof _.settings.section_content === 'function') excerpt = _.settings.section_content.call(self, result, section_name, url);
				...
				if(pos1 == -1 || pos2 == -1) {
					help_modal.find('.model-title').html( '&nbsp;' );
					return;
				}
		...
		.fail(function() {
			help_modal.find('.model-title').find('.fa-spin').remove().end().find('.hidden').children().unwrap();
		});
	}//launch_help_modal


	$(document).on('click', '.onpage-help-model a[href^="http"]', function() {
	...
	function display_codeview(url, language, save_to_list) {
		...
		.done(function(result) {
			...
			help_modal.find('.model-title').html($url).wrapInner('<code />');
